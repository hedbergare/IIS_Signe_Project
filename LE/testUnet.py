...
class UNetModel(nn.Module):
        ...
        for layer in range(self.depth-1):
            ...
            if self.current_layer == 1:
                self.decBlocks.append(
                    Block(self.blockFactor, self.output_channels))
            else:
    ...
    def forward(self, x):
        ...
        for i in range(len(self.decBlocks)):
            x = self.upconvs[i](x)
            # encodingFeatures = self.cropthing(
            #    encodingFeatures[len(encodingFeatures)-(1+i)], x)
            encodingFeatures = self.cropthing2(
                encodingFeatures[len(encodingFeatures)-(1+i)], x)
            print('x', x.shape)
            print(len(encodingFeatures))
            print(encodingFeatures[len(encodingFeatures)-(1+i)].shape)
            x = torch.cat((x, encodingFeatures[len(encodingFeatures)-(1+i)]))
            x = self.decBlocks
        return x
